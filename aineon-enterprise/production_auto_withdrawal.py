...
class ProductionAutoWithdrawal:
    ...
    def execute_real_withdrawal(self, amount):
        ...
        if not self.web3 or not self.private_key:
            logger.warning("⚠️ Web3 not available - simulating withdrawal")
    ...
    def run_withdrawal_cycle(self):
        ...
        if not can_withdraw:
            logger.info(f"⏸️ Withdrawal not executed: {reason}")
    ...
    def start_continuous_monitoring(self):
        ...
        logger.info(f"🛡️ Safety Buffer: {self.safety_buffer_eth} ETH")
...
def main():
    ...
    print("\n⚠️  WARNING: This system will make actual blockchain transactions!")
    print("⚠️  Ensure your private key is configured in .env file")
